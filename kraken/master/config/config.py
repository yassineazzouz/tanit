...
class MasterConfig(Config):
    ...
    def load(self):
        ...
        if (self.config.has_option("master", "worker-service_address")):
            ...
            raise KrakenConfigurationException("Missing master rpc worker service address from configuration")
        
        if(len(worker_service_address.split(":")) != 2):
            raise KrakenConfigurationException("Master rpc worker service address should be formatted as host:port.")
        ...
        if (self.config.has_option("master", "client-service_address")):
            ...
            raise KrakenConfigurationException("Missing master rpc client service address from configuration")
        
        if(len(client_service_address.split(":")) != 2):
            raise KrakenConfigurationException("Master rpc client service address should be formatted as host:port.")
