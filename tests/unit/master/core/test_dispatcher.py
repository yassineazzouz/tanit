...
class TestSimpleDispatcher:
    ...
    def test_simple_dipstacher(self, simple_dispatcher):
        ...
        simple_dispatcher.workers_manager.register_worker(
            mock_worker("worker 1", 10)
        )
        ...
        assert wait_until(
            _verify_queue_size, 10, 0.5, simple_dispatcher.cqueue, 0
        )
...
class TestFairDispatcher:
    ...
    def test_dispatcher_fairness_2(self, simple_dispatcher):
        ...
        """
        simple_dispatcher.workers_manager.register_worker(
            mock_worker("worker 1", 5)
        )
        simple_dispatcher.workers_manager.register_worker(
            mock_worker("worker 2", 12)
        )
