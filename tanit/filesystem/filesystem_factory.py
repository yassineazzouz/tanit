...
class FileSystemFactory(object):
    ...
    def create_filesystem(self, config):
        if "type" not in config:
            raise NonSupportedFileSystemError("filesystem type missing for '%s'" % config)
        else:
            ...
            if fs_type == "local":
                ...
                filesystem = HDFSFileSystem(auth_mechanism, **config)
            elif fs_type == "s3":
